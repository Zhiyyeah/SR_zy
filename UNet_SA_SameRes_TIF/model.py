...
class SpatialAttention(nn.Module):
    """CBAM-style spatial attention: avg+max pool across channels -> 7x7 conv -> sigmoid."""
...
def _best_gn_groups(ch: int, preferred: int) -> int:
    """Pick a GroupNorm group count that divides 'ch'. Fallback to 1 if needed."""
    g = min(preferred, ch)
...
class SAResBlock(nn.Module):
    """Spatial-Attention Residual Block (keeps H×W)."""
...
class UNetSASameRes(nn.Module):
    ...
    - No pooling/upsampling; H×W preserved from input to output.
    - Sequence of SA-ResBlocks (optionally dilated) to aggregate context.
    - Global residual adds a 1×1 projection of input to the network output.
    """
...
def create_model(in_channels: int = 5, out_channels: int = 5) -> nn.Module:
    """Factory for the same-resolution SR model (no upsampling)."""
...
if __name__ == "__main__":
    ...
    print("[Model Info]")
    print(f"Input shape: {tuple(x.shape)}  Output shape: {tuple(y.shape)}  Residual: {RESIDUAL}")
